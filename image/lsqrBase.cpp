...
SolveParallel(unsigned int m, unsigned int n, const double* b, double* x, bool clear_x)
{
	...
	double* tmp_buffer = new double[omp_get_max_threads()];
...
#pragma omp parallel
	{
		...
		if (this->wantse)
		{
			std::fill(se + start_n, se + end_n, zero);
		}

		double alpha = zero;

		tmp_buffer[t] = this->Dnrm2(end_m - start_m, u + start_m);
#pragma omp barrier
		double beta = this->Dnrm2(threads, tmp_buffer);
#pragma omp barrier
		...
		if (beta > zero)
		{
			...
			this->Aprod2(m, n, v, u);   //     v = A'*u
			tmp_buffer[t] = this->Dnrm2(end_n - start_n, v + start_n);
#pragma omp barrier
			alpha = this->Dnrm2(threads, tmp_buffer);
#pragma omp barrier
		}
...
#pragma omp barrier
				...
				tmp_buffer[t] = this->Dnrm2(end_m - start_m, u + start_m);
#pragma omp barrier
				beta = this->Dnrm2(threads, tmp_buffer);
#pragma omp barrier
...
#pragma omp barrier
				...
				tmp_buffer[t] = dknorm;
#pragma omp barrier
				dknorm = tmp_buffer[0]; for (unsigned int i = 1; i < threads; i++) dknorm += tmp_buffer[i];
#pragma omp barrier
